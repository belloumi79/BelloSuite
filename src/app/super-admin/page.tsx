...
export default function SuperAdminDashboard() {
  ...
  async function fetchData() {
    try {
      ...
      let mrr = 0
      for (const tm of (tenantModulesData || []) as any[]) {
        if (tm.is_enabled && tm.module) mrr += tm.module.monthly_price || 0
      }

      setStats({
        totalClients: tenantsData?.length || 0,
        totalUsers: usersCount || 0,
        activeModules: activeModuleIds.size,
        totalModules: modulesData?.length || 6,
        mrr,
      })
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-8 h-8 rounded-full bg-emerald-500/20 flex items-center justify-center">
              <span className="text-emerald-400 text-sm font-medium">BK</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="text-left text-xs text-zinc-500 uppercase tracking-wider mb-4">
                    <th className="pb-4 font-medium">Entreprise</th>
                    <th className="pb-4 font-medium">Slug</th>
                    <th className="pb-4 font-medium">Modules</th>
                    <th className="pb-4 font-medium">Utilisateurs</th>
                ...
                <tbody className="divide-y divide-zinc-800/50">
                  {tenants.map((tenant) => (
                    <tr key={tenant.id} className="text-sm">
                      ...
                      <td className="py-4">
                        <span className={`px-2 py-1 rounded-lg text-xs font-medium ${tenant.is_active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                  ...
                  {tenants.length === 0 && (
                    <tr><td colSpan={6} className="py-12 text-center text-zinc-500">Aucun client pour le moment</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-white">Modules Disponibles</h2>
            <button className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl text-sm font-medium transition-colors">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
              Nouveau Module
            </button>
          </div>
              ...
              <div key={mod.name} className="bg-zinc-900/50 backdrop-blur-xl rounded-2xl border border-zinc-800/50 p-6 hover:border-zinc-700/50 transition-colors">
                <div className="flex items-start justify-between mb-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${i % 2 === 0 ? 'bg-emerald-500/10' : 'bg-blue-500/10'}`}>
                    <span className={`text-xl ${i % 2 === 0 ? 'text-emerald-400' : 'text-blue-400'}`}>
                      {mod.name === 'stock' && '📦'}
                      {mod.name === 'commercial' && '💼'}
                      {mod.name === 'accounting' && '💰'}
                      {mod.name === 'hr' && '👥'}
                  ...
                  <span className={`px-2 py-1 rounded-lg text-xs font-medium ${i < 4 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-zinc-800 text-zinc-400'}`}>
                    {i < 4 ? 'Actif' : 'Inactif'}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-zinc-800/50">
                  <div>
                    <p className="text-2xl font-bold text-white">{mod.monthly_price} TND</p>
                    <p className="text-zinc-500 text-xs">/mois</p>
                  </div>
                  <div className="text-right">
                    <p className="text-white font-medium">{Math.floor(Math.random() * 15) + 1}</p>
                    <p className="text-zinc-500 text-xs">clients</p>
                  </div>
                </div>
                <div className="mt-4 flex gap-2">
                  <button className="flex-1 px-3 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg text-sm font-medium transition-colors">Modifier</button>
                  <button className="flex-1 px-3 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 rounded-lg text-sm font-medium transition-colors">Voir Clients</button>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className={`bg-gradient-to-br ${colors[color]} rounded-2xl p-6 border backdrop-blur-xl`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${colors[color].split(' ')[0]} flex items-center justify-center`}>
          <svg className={`w-5 h-5 ${icons[color]}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
