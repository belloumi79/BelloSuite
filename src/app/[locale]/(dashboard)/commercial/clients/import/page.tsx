...
import { Upload, FileSpreadsheet, AlertCircle, CheckCircle, X, Download, Table, Users } from 'lucide-react'

const TEMPLATE_FIELDS = [
  'code', 'name', 'email', 'phone', 'address', 'city',
  'zipCode', 'country', 'matriculeFiscal',
]

const SAMPLE = {
  code: 'CLI-001',
  name: 'Société ABC S.A.',
  email: 'contact@abcdistribuoid.tn',
    ...
    <div className="p-8 space-y-8 max-w-5xl mx-auto min-h-screen bg-transparent pt-0">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-white tracking-tight">Import Clients</h1>
      ...
      {file && (
        <button
          ...
          disabled={loading}
          className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:bg-zinc-700 text-white rounded-2xl font-black text-base transition-all flex items-center justify-center gap-3"
        >
          {loading ? (
            <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Import en cours...</>
          ) : (
            <><Upload className="w-5 h-5" /> Importer {preview.length} clients</>
          )}
        </button>
      )}
                  ...
                  <div className="space-y-1">
                    {results.log.slice(0, 10).map((l: any, i: number) => (
                      <div key={i} className="flex items-center gap-3 text-xs font-mono text-zinc-400 bg-zinc-800/30 rounded-lg px-3 py-2">
                        ...
                        <span className="text-emerald-400">{l.code}</span>
