...
const MODULE_REGISTRY: Record<string, { tKey: string; icon: any; path: string; alwaysVisible?: boolean }> = {
  dashboard:   { tKey: 'Common.dashboard', icon: LayoutDashboard, path: '/fr/dashboard', alwaysVisible: true },
  stock:       { tKey: 'Home.modules.stock.title', icon: Package, path: '/fr/stock' },
  commercial:  { tKey: 'Home.modules.commercial.title', icon: ShoppingCart, path: '/fr/commercial' },
  accounting:  { tKey: 'Home.modules.accounting.title', icon: Wallet, path: '/fr/accounting/chart' },
  hr:          { tKey: 'Home.modules.hr.title', icon: Users, path: '/fr/hr' },
  pos:         { tKey: 'Home.modules.commercial.title', icon: ShoppingCart, path: '/fr/pos' },
  gmao:        { tKey: 'Home.modules.maintenance.title', icon: Wrench, path: '/fr/gmao' },
  gpao:        { tKey: 'Home.modules.production.title', icon: Factory, path: '/fr/gpao' },
}
        ...
        {loading ? (
          ...
          <>
            {sidebarItems.map((module) => {
              const isActive = pathname === module.path || (module.path !== '/fr/dashboard' && pathname.startsWith(module.path))
              return (
                <Link
                  key={module.key}
                  href={module.path}
