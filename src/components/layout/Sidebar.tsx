...
import {
  Package, ShoppingCart, Users, LayoutDashboard, LogOut,
  ChevronLeft, ChevronRight, Wallet, Wrench, Factory, Lock,
  BarChart3, Receipt as POSIcon,
} from 'lucide-react'
  ...
  useEffect(() => {
    fetch('/api/auth/session')
      ...
      .then(data => {
        ...
        if (data?.tenantId) {
          return fetch(`/api/tenant/modules?tenantId=${data.tenantId}`)
            .then(r => r.json())
            .then(setEnabledModuleNames)
  ...
  return (
    <aside className={`bg-zinc-950 border-inline-end border-zinc-800/50 flex flex-col transition-all duration-500 ease-in-out h-screen sticky top-0 z-[60] ${isCollapsed ? 'w-20' : 'w-72'}`}>
      <div className="p-6 flex items-center justify-between shrink-0">
        ...
        {isCollapsed && (
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center mx-auto">
            <span className="text-white font-black text-xl">B</span>
          </div>
        )}
        <button onClick={() => setIsCollapsed(!isCollapsed)} className="p-2 hover:bg-zinc-900 rounded-xl text-zinc-400 transition-colors hidden sm:block">
        ...
        {loading ? (
          ...
          <>
            {sidebarItems.map((module) => {
              ...
              return (
                <Link key={module.key} href={module.path} className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition-all group relative overflow-hidden ${isActive ? 'bg-teal-500/10 text-teal-400' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900/50'}`}>
                  {isActive && <div className="absolute inset-inline-start-0 w-1.5 h-6 bg-teal-500 rounded-inline-end-full" />}
                  <module.icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-teal-400' : ''}`} />
                  {!isCollapsed && <span className="font-semibold text-sm tracking-wide grow">{t(module.tKey as any)}</span>}
                </Link>
              )
            })}
